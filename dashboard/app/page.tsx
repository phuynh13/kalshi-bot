          ...
          {error ? (
            ...
            <div className="overflow-x-auto">
              ...
              <div className="mt-3 flex justify-end">
                <button onClick={load} className="text-xs text-gray-500 hover:text-gray-300 border border-gray-700 rounded-lg px-3 py-1 transition">
                  Refresh
                </button>
              </div>
            </div>
...
function RunFunnel({ run, ordersForRun }: { run: Run | null; ordersForRun: Order[] }) {
  const [expanded, setExpanded] = useState(false);
  if (!run) return null;
  ...
  for (const o of ordersForRun) {
    if (o.yes_ask_dollars && o.yes_bid_dollars) spreads.push(o.yes_ask_dollars - o.yes_bid_dollars);
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-xl mb-6 overflow-hidden">
      ...
      {expanded && (
        <div className="px-5 pb-5 border-t border-gray-800 grid grid-cols-1 md:grid-cols-3 gap-5 pt-4">
          <div>
            <h3 className="text-xs text-gray-500 uppercase tracking-wider mb-2">Why markets were filtered out</h3>
              ...
              <ul className="space-y-1.5 text-sm">
                {filterReasons.map(([reason, count]) => (
                  <li key={reason} className="flex items-center justify-between">
                    <span className="text-gray-400 text-xs">{reason.replace(/_/g, " ")}</span>
                    <span className="font-mono text-gray-300">{Number(count).toLocaleString()}</span>
                  </li>
                ))}
              </ul>
            )}
          ...
          <div>
            <h3 className="text-xs text-gray-500 uppercase tracking-wider mb-2">Placement loop</h3>
          ...
          <div>
            <h3 className="text-xs text-gray-500 uppercase tracking-wider mb-2">Placed-order stats</h3>
            {ordersForRun.length === 0 ? (
              ...
              <ul className="space-y-1.5 text-sm">
                <li className="flex items-center justify-between">
                  ...
                  <span className="font-mono text-gray-300 text-xs">${minOf(midpoints)?.toFixed(3)} – ${maxOf(midpoints)?.toFixed(3)}</span>
                </li>
                <li className="flex items-center justify-between">
                  <span className="text-gray-400 text-xs">avg midpoint</span>
                  <span className="font-mono text-gray-300 text-xs">${avg(midpoints)?.toFixed(4)}</span>
                </li>
                <li className="flex items-center justify-between">
                  <span className="text-gray-400 text-xs">spread range</span>
                  <span className="font-mono text-gray-300 text-xs">${minOf(spreads)?.toFixed(3)} – ${maxOf(spreads)?.toFixed(3)}</span>
                </li>
                <li className="flex items-center justify-between">
                  <span className="text-gray-400 text-xs">avg spread</span>
                  <span className="font-mono text-gray-300 text-xs">${avg(spreads)?.toFixed(4)}</span>
                </li>
              </ul>
            )}
          </div>
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 mb-6">
      <div className="flex items-center justify-between mb-1">
        <h2 className="text-sm font-medium text-gray-400">Edge over implied odds</h2>
        <span className="text-xs text-gray-600">fade = small sample (&lt;{MIN_RELIABLE_SAMPLE})</span>
      </div>
      <p className="text-xs text-gray-600 mb-4">
        Actual win rate minus the market's implied probability at each entry price.
        Labels show edge % and sample size. Zero = exactly what the market predicted.
      </p>

      {!hasData ? (
        <p className="text-sm text-gray-500 py-8 text-center">No settled orders yet.</p>
      ) : (
        <>
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl mb-6 overflow-hidden">
      <div className="px-5 py-4 border-b border-gray-800">
        <h2 className="text-sm font-medium text-gray-400">Performance by category</h2>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-800">
            {stats.map((s) => {
              ...
              const pnlColor = s.net_pnl == null ? "text-gray-400" : s.net_pnl >= 0 ? "text-emerald-400" : "text-red-400";
              const winRateColor = s.win_rate_pct == null ? "text-gray-400" : s.win_rate_pct >= 58 ? "text-emerald-400" : "text-red-400";
              return (
                <tr key={s.category} className={`hover:bg-gray-800/40 transition ${reliable ? "opacity-100" : "opacity-40"}`}>
                  <td className="px-4 py-3 text-gray-300 whitespace-nowrap">{s.category}</td>
                  <td className="px-4 py-3 font-mono text-gray-300">{s.settled_orders}</td>
                  <td className="px-4 py-3 font-mono text-emerald-400 text-xs">{s.wins}</td>
                  <td className="px-4 py-3 font-mono text-red-400 text-xs">{s.losses}</td>
                  <td className={`px-4 py-3 font-mono ${winRateColor}`}>{fmt.pct(s.win_rate_pct)}</td>
                  <td className="px-4 py-3 font-mono text-gray-400 text-xs">${s.total_spent?.toFixed(2) ?? "0.00"}</td>
                  <td className="px-4 py-3 font-mono text-gray-500 text-xs">${s.total_fees?.toFixed(4) ?? "0.0000"}</td>
                  <td className={`px-4 py-3 font-mono font-medium ${pnlColor}`}>{fmt.dollars(s.net_pnl)}</td>
                </tr>
              );
            })}
          </tbody>
...
function ResultBadge({ result }: { result: string | null }) {
  if (!result) return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs bg-gray-800 text-gray-400">Pending</span>;
  if (result === "yes") return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs bg-emerald-900 text-emerald-300 font-medium">YES ✓</span>;
  if (result === "cancelled") return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs bg-yellow-900 text-yellow-300">Expired unfilled</span>;
  if (result === "unknown") return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs bg-gray-700 text-gray-300">Unresolved</span>;
  return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs bg-red-900 text-red-300 font-medium">NO ✗</span>;
}
...
function PnLChart({ data }: { data: DailyPnl[] }) {
  ...
  const chartData = sorted.map((d) => {
    ...
    return { date: d.trade_date, cumulative: parseFloat(cumulative.toFixed(4)) };
  });
  const isPositive = cumulative >= 0;

  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
      <h2 className="text-sm font-medium text-gray-400 mb-4">Cumulative P&amp;L (after fees)</h2>
      <ResponsiveContainer width="100%" height={220}>
        <AreaChart data={chartData}>
          ...
          <YAxis tick={{ fill: "#6b7280", fontSize: 11 }} tickLine={false} axisLine={false} tickFormatter={(v) => `$${v.toFixed(2)}`} />
          <Tooltip
            contentStyle={{ backgroundColor: "#111827", border: "1px solid #374151", borderRadius: "8px", fontSize: "12px" }}
            formatter={(v: number) => [`$${v.toFixed(4)}`, ""]}
          />
          <Area type="monotone" dataKey="cumulative" stroke={isPositive ? "#10b981" : "#ef4444"} strokeWidth={2} fill="url(#pnlGradient)" name="Cumulative P&L" />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
...
function OrdersTable({ orders }: { orders: Order[] }) {
  ...
  const missingFillCount = orders.filter(
    (o) =>
      ...
      (o.fill_cost_dollars == null || o.filled_count == null)
  ).length;
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
          ...
          <p className="text-xs text-gray-600 mt-0.5">
            ...
            {missingFillCount > 0 && (
              <span className="ml-2 text-amber-600">
                · {missingFillCount} settled orders using estimated fill price (see ~)
              </span>
            )}
          </p>
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-800">
            {pageOrders.length === 0 && (
              <tr>
                <td colSpan={8} className="px-4 py-8 text-center text-gray-500">No orders yet.</td>
              </tr>
            )}
            {pageOrders.map((o) => {
              const netPnl = o.pnl_dollars == null ? null : o.pnl_dollars - (o.fees_dollars ?? 0);
              ...
              if (o.fill_cost_dollars != null && o.filled_count) {
                ...
                filledAtDisplay = <span className="text-gray-600">—</span>;
              }
      ...
      {totalPages > 1 && (
        <div className="px-5 py-3 border-t border-gray-800 flex items-center justify-between">
          <span className="text-xs text-gray-600">{orders.length.toLocaleString()} total orders</span>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setPage((p) => Math.max(0, p - 1))}
              disabled={page === 0}
...
export default function DashboardPage() {
  ...
  async function load() {
    ...
    const [statsRes, dailyRes, ordersRes, runsRes, catRes] = await Promise.all([
      ...
      supabase.from("daily_pnl").select("*").order("trade_date", { ascending: false }).limit(30),
      supabase.from("orders").select("*").order("created_at", { ascending: false }).limit(500),
      supabase.from("runs").select("*").order("run_at", { ascending: false }).limit(1),
  ...
  const ordersForLastRun = lastRun ? orders.filter((o) => o.run_id === lastRun.id) : [];
  const pnlColor = !stats || stats.total_pnl == null ? "text-white" : stats.total_pnl >= 0 ? "text-emerald-400" : "text-red-400";
  const preFeeStr = stats?.total_pnl_pre_fees != null
    ? `${stats.total_pnl_pre_fees >= 0 ? "+" : ""}$${Math.abs(stats.total_pnl_pre_fees).toFixed(2)} pre-fee`
    : "";
  ...
  return (
    <div className="min-h-screen px-4 py-8 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-semibold text-white">Kalshi Bot Dashboard</h1>
          <p className="text-sm text-gray-500 mt-0.5">Automated YES limit orders · 58–85% probability range</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <StatCard
          ...
          sub={`${stats?.settled_orders ?? 0} settled · ${stats?.cancelled_orders ?? 0} expired · ${stats?.pending_orders ?? 0} pending`}
        />
        <StatCard
          label="Win Rate"
          value={fmt.pct(stats?.win_rate_pct)}
          sub={`${stats?.total_wins ?? 0}W / ${stats?.total_losses ?? 0}L`}
          color={stats?.win_rate_pct == null ? "text-white" : stats.win_rate_pct >= 58 ? "text-emerald-400" : "text-red-400"}
        />
        <StatCard
          label="Net P&L"
